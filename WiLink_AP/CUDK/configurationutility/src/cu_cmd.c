...
{    
    ...
    { FILE_ID_139 ,  (PS8)"rolesMgr                " },            
    { FILE_ID_140 ,  (PS8)"roleAP                  " },              
    { FILE_ID_141 ,  (PS8)"wlanLinks               " },           
    { FILE_ID_142 ,  (PS8)"ApCmd                   " },		          
...
VOID CuCmd_ApRoleDbgBssStart(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    ...
    if (nParms != 1)
    {
        os_error_printf(CU_MSG_INFO2, (PS8)"Param 0 - Bss Index (0,1,2,..7)\n");
    }
    else
    {
        bssIdx = parm[0].value;
        CuCommon_SetBuffer(pCuCmd->hCuCommon, ROLE_AP_BSS_START_PARAM, (PVOID)&bssIdx, sizeof(bssIdx));
    }
}

VOID CuCmd_ApRoleDbgBssStop(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    CuCmd_t* pCuCmd = (CuCmd_t*)hCuCmd;
    TI_UINT8 bssIdx;
    ...
    {
        bssIdx = parm[0].value;
        CuCommon_SetBuffer(pCuCmd->hCuCommon, ROLE_AP_BSS_STOP_PARAM, (PVOID)&bssIdx, sizeof(bssIdx));
    }
}


VOID CuCmd_ApRoleSetSSIDType(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    CuCmd_t* pCuCmd = (CuCmd_t*)hCuCmd;
    ...
    if (nParms != 1)
    {
        os_error_printf(CU_MSG_INFO2, (PS8)"Param 0 - SSID type (0=public, 1=hidden)\n");
    }
    else
    {
        ssidType = (SsidType_enum)parm[0].value;
        CuCommon_SetBuffer(pCuCmd->hCuCommon, ROLE_AP_SET_SSID_TYPE_PARAM, (PVOID)&ssidType, sizeof(ssidType));
    }

}


VOID CuCmd_ApRoleSetChannel(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    CuCmd_t* pCuCmd = (CuCmd_t*)hCuCmd;
    TI_UINT8 channel;
    ...
    {
        channel = parm[0].value;
        CuCommon_SetBuffer(pCuCmd->hCuCommon, ROLE_AP_SET_CHANNEL_PARAM, (PVOID)&channel, sizeof(channel));
    }
}
...
VOID CuCmd_ApRoleSetBeconInterval(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    ...
    {
        interval = parm[0].value;
        CuCommon_SetBuffer(pCuCmd->hCuCommon, ROLE_AP_SET_BEACON_INTERVAL_PARAM, (PVOID)&interval, sizeof(interval));
    }
}
...
VOID CuCmd_ApRoleSetDTIMInterval(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    ...
    if (nParms != 1)
    {
        os_error_printf(CU_MSG_INFO2, (PS8)"Param 0 - DTIM interval \n");
    }
    else
    {
        interval = parm[0].value;
        CuCommon_SetBuffer(pCuCmd->hCuCommon, ROLE_AP_SET_DTIM_PERIOD_PARAM, (PVOID)&interval, sizeof(interval));
    }
}
...
VOID CuCmd_ApRoleSetSSID(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    ...
    {
        os_error_printf(CU_MSG_INFO2, (PS8)"Param 0 <SSID> is limited up to 32 chars \n");
    }
    else
    {
        ssid.SsidLength = os_strlen( (PS8)parm[0].value);
        os_memcpy((PVOID)ssid.Ssid, (PVOID) parm[0].value, ssid.SsidLength);
        CuCommon_SetBuffer(pCuCmd->hCuCommon, ROLE_AP_SET_SSID_PARAM, (PVOID)&ssid, sizeof(ssid));
    }
...
VOID CuCmd_ApRoleUpdateKeyInConfFile(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    ...
    if (nParms == 0)
    {
        os_error_printf(CU_MSG_INFO2, (PS8)"\nAp Role configuration menu: <index> <param0> <param1> ... <param n> i.e. 1 MySSID \n");
        os_error_printf(CU_MSG_INFO2, (PS8)"To delete parameter type <index> with empty value\n\n");
        CuHostapd_PrintMenu();
        return;
    }
    else
    {
        uKeyIdx = parm[0].value - 1;
        uKeyLength = os_strlen((PS8)hostapdKeysList[uKeyIdx].keyName);
        os_memcpy((PVOID)keyValPair.key, (PVOID)hostapdKeysList[uKeyIdx].keyName, uKeyLength);
        keyValPair.key[uKeyLength] = '\0';

        valueLength = 0;
        for (i = 1 ; i < nParms ; i++)
        {
            for (j = 0; j< os_strlen((PS8)parm[i].value); j++)
            {
                keyValPair.value[valueLength++] = ((TI_UINT8*)parm[i].value)[j];
            }

            keyValPair.value[valueLength++]=' ';
        }


        valueLength--;
        keyValPair.uValueLength = valueLength;
        keyValPair.uKeyLength = uKeyLength;
        keyValPair.value[valueLength]='\0';

        CuHostapd_UpdateKeyInDictionary(&keyValPair);
    }
...
VOID CuCmd_ApRoleSendCmdToHostapd(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    ...
    if (nParms == 0)
    {
        os_error_printf(CU_MSG_INFO2, (PS8)"Hostapd commands available: \n");
#ifdef TI_HOSTAPD_CLI_LIB
        HostapdCLI_PrintCommands();
#endif
        return;
    }
    else
    {
        cmd.eCmdType = (EHostapdCLICmds)(parm[0].value - 1);
		...
		case HOSTAPD_CLI_CMD_START:
            os_memcpy((PVOID)cmd.u.tCmdStart.config_fname, (PVOID)parm[1].value, os_strlen((PS8)parm[1].value));
			break;
...
VOID CuCmd_SendStartCmdToHostapd(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    ...
    if (nParms != 1)
    {
        os_error_printf(CU_MSG_INFO2, (PS8)"1 input parameter required - config file name \n");
        return;
    }
...
VOID CuCmd_ApRolePrintDb(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    ...
    if (nParms != 0)
    {
        os_error_printf(CU_MSG_INFO2, (PS8)"No input parameters required \n");
    }
    else
    {
        CuCommon_SetBuffer(pCuCmd->hCuCommon, ROLE_AP_PRINT_DB, NULL, 0);
    }
}
